...
rc('text', usetex=True)
#rc('text.latex', preamble=r'\usepackage{tikz}\usepackage{lmodern}')
...
def plot_all_tests(
    ...
    **kwargs
    ...
    # else:
    #     df = pd.DataFrame(
    #         {'Score': scores, 'Metric': metric_names, 'Type': metric_type,
    #          'ProbBased': prob_based})
    #     plot_barplot(df, f'{core_name}-{plot_type}-metrics.pdf', out_dir)
...
def plot_groups(
    ...
    for i, (data, mcounts) in enumerate(zip(dfs, metric_counts)):
        ...
        cbar = False
        # if the data is scaled to 0, 1, setting the range to [-1, 1]
        # causes the loss of blue highlighting
        vmin, vmax = -1, 1
...
def _get_group_scores(test, group2scores, res_munch, cl):
    ...
    if cl in results:
        ...
        for name, score in name2score.items():
            ...
            # the metric is not meant to be accumulates
