import numpy as np
from checklist_fork.checklist.utils import iter_with_optional, is_1d_list, \
    compare_preds_with_mismatched_tokens
import itertools
...
class Expect:
    ...
    def test(fn):
        ...
        -------
        function
            Arguments: EvaluationCore
            Returns: List of np.arrays
        """
        def expect(self):
            return fn(self.data, self.preds, self.confs, self.labels, self.meta, self.run_idxs)
        return expect
    ...
    def testcase(fn):
        ...
        """
        def expect(self):
            zipped = iter_with_optional(self.data, self.preds, self.confs, self.labels, self.meta, self.run_idxs)
    ...
    @staticmethod
    def pairwise(fn, allow_list_examples=False):
        ...
        def expect_fn(xs, preds, confs, labels=None, meta=None):
            ...
            # seq labeling
            final_ret = []
            if type(orig_pred) in [list, np.ndarray] and \
                    not allow_list_examples:
                for _, p, c, l, m in iter_with_optional(
                        xs, preds, confs, labels, meta):
                    res = []
                    if l is None:
                        l = itertools.repeat(l)

                    for i, (token_pred, token_conf, token_label) \
                            in enumerate(zip(p, c, l)):
                        res.append(fn(orig_pred[i], token_pred, orig_conf[i],
                                    token_conf, token_label, m))

                    # TODO: perhaps the condition in the list comprehension
                    # should be different
                    final_ret.append(all([x == True for x in res]))
                return np.array(final_ret)
            else:
                return np.array(
    ...
    def eq(val=None):
        ...
        def ret_fn(x, pred, conf, label=None, meta=None):
            ...
            # TODO: this is added to handle 'no class' in the label, but it
            # breaks MFTs for now: it messes up the % of tests  passed/failed
            # and should be fixed
            if pred == gt or val is None:
                return True
            elif is_1d_list(gt):
                if len(gt) == len(pred):
                    # not the same but the same length
                    return False
                else:
                    # there are token mismatches
                    if not meta:
                        raise Exception("Cannot compare two sequences ")
                    return compare_preds_with_mismatched_tokens(pred, gt, meta)

            elif type(gt) == str:
                # if label is a string we can't get the confidence violation
                return False
            else:
                softmax = type(conf) in [np.ndarray]
    ...
    @staticmethod
    def inv(tolerance=0, allow_list_examples=False):
        ...
        def expect(orig_pred, pred, orig_conf, conf, labels=None, meta=None):
            softmax = type(orig_conf) in [np.ndarray]
            try:
                if pred == orig_pred:
                    return True
            ...
            if is_1d_list(pred):
                if len(orig_pred) == len(pred):
                    ...
                    if not meta:
                        raise Exception("Cannot compare two sequences ")
                    return compare_preds_with_mismatched_tokens(
                        pred, orig_pred, meta)
        ...
        return Expect.pairwise(expect, allow_list_examples)
