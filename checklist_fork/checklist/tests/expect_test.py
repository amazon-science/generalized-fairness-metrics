...
from ..utils import iter_with_optional, adjust_for_class_eval, DataShape, \
    is_2d_list, to_np_array, convert_all_preds_into_ints
        ...
        def default_format_example(
            ...
            if softmax:
                if conf.shape[0] == 2:
                    ...
                    if type(pred) in [list, np.ndarray]:
...
#             adjust_for_class_eval(
#                 core_record, self.n_classes, filter_none=False)
...
class DIR(ExpectTest):
    def __init__(self, expect, agg_fn='all_except_first', name=None):
