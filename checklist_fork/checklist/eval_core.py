...
class EvaluationCore():
    ...
    def _filter_the_data(self, data_filter: Dict):
        # TODO: all of this works if the data wasn't sampled
        # sampling hasn't been addressed yet
        indx_to_keep = []
    ...
    def run_from_preds_confs(
        ...
        for test in self.tests:
            try:
                ...
                # this test updates the self.results field with the new results
    ...
    def run_from_file(
        ...
        # if "seq_" in file_format:
        #     self.lables = self._labels_dict[""]
    ...
    #     TODO: this function doesn't support sequence labelling (which
    #     requires label vocabulary to be provided as well)
    #     """
        ...
        for i, test in enumerate(self.tests):
            ...
            try:
                ...
                if tname not in self.results:
                    ...
                    core_record = self._get_core_record(test)
                    # logger.info(
                    #     f"Evaluated on {len(core_record.data)} examples " +
                    #     f"that meet: {self.results.data_filter}")
